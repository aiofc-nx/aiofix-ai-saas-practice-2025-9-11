...
- **分层设计**：四层架构（Domain、Application、Interface、Infrastructure）、职责分明
- **依赖倒置**：通过接口和抽象类实现依赖倒置
...
```
┌─────────────────────────────────────────┐
│             Interface Layer             │  ← RESTful API、GraphQL API、gRPC API、消息队列接口
├─────────────────────────────────────────┤
│            Application Layer            │  ← 用例、应用服务、命令处理器、查询处理器、事件处理器
├─────────────────────────────────────────┤
│              Domain Layer               │  ← 实体、值对象、领域服务
├─────────────────────────────────────────┤
│           Infrastructure Layer          │  ← 数据访问、外部服务、AI集成
└─────────────────────────────────────────┘
```
...
- **消息队列接口**：消息队列的发布和订阅接口
- **DTO**：数据传输对象，协议数据与内部模型的转换

##### Application Layer（应用层）

- **用例（Use Cases）**：业务流程的编排和协调
- **应用服务**：跨聚合的业务逻辑处理
- **命令处理器（Command Handlers）**：处理业务命令，执行写操作
- **查询处理器（Query Handlers）**：处理查询请求，执行读操作
- **事件处理器（Event Handlers）**：处理领域事件，实现事件驱动逻辑
- **事件订阅器（Event Subscribers）**：订阅和处理外部事件
...
- **领域层**：业务实体、值对象、领域服务、领域事件
- **应用层**：用例、应用服务、数据传输对象
- **基础设施层**：数据访问、外部服务、AI服务集成
- **表现层**：API控制器、用户界面
